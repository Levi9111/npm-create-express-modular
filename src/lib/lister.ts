...
function _isModuleWired(projectRoot: string, moduleName: string): boolean {
...
export function listProject(): void {
  ...
  if (fs.existsSync(modulesDir)) {
    ...
    if (modules.length === 0) {
      ...
      modules.forEach((mod) => {
        ...
        const wired = isWired ? ui.green('● wired') : ui.yellow('○ not wired');
  ...
  console.log();
  console.log(
    `  ${ui.bold(ui.cyan('◆  Middlewares'))}  ${ui.gray('src/app/middlewares/')}`,
  );
  console.log();
  ...
  if (fs.existsSync(mwDir)) {
    ...
    if (coreMw.length > 0) {
      coreMw.forEach((f) =>
        console.log(`     ${ui.gray('◇')}  ${ui.gray(f)}  ${ui.gray('[core]')}`),
      );
    }
    if (customMw.length === 0) {
      console.log(
        `     ${ui.gray('(no custom middlewares — run: cem add middleware <name>)')}`,
      );
    } else {
  ...
  console.log();
  console.log(
    `  ${ui.gray('Tip:')}  ${ui.gray('cem remove module <Name> | cem remove middleware <name> | cem remove env <KEY>')}`,
  );
  console.log();
}
