 ...
 */
function scaffoldAuth(projectPath, db = 'mongoose', validator = 'zod', tokenDelivery = 'cookie', pm = 'npm') {
...
`,
    ...
    fs.writeFileSync(
        ...
        _buildAuthSetupGuide(db, tokenDelivery, pm),
    );
...
// ─── MODEL ────────────────────────────────────────────────────────────────────
function _buildModelReal(db, pm = 'npm') {
    const execMap = {
        npm: 'npx',
        yarn: 'yarn dlx',
        pnpm: 'pnpm dlx'
    ...
    switch (db) {
        case 'mongoose':
            return `import { Schema, model, CallbackError } from 'mongoose';
import bcrypt from 'bcrypt';
...
// ─── AUTH SETUP GUIDE ─────────────────────────────────────────────────────────
function _buildAuthSetupGuide(db, tokenDelivery, pm = 'npm') {
    const execMap = {
        npm: 'npx',
        yarn: 'yarn dlx',
        pnpm: 'pnpm dlx'
