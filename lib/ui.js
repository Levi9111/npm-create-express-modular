 ...
 * @param {{ name: string, db: string, validator: string, auth: boolean, docker: boolean }} opts
 */
function printSummary({ name, db, validator, auth, docker }) {
...
// ─── UPDATE NOTICE ───────────────────────────────────────────────────────────
function printUpdateNotice(current, latest, pm = 'npm') {
  const W = 52;
  const border = (l, m, r) => `  ${yellow(l + '─'.repeat(W) + r)}`;
  const blankRow = () => `  ${yellow('│')}${' '.repeat(W)}${yellow('│')}`;
  ...
  const globalCmd = { npm: 'npm i -g', yarn: 'yarn global add', pnpm: 'pnpm add -g' }[pm] || 'npm i -g';
