...
const ui = require('./ui');
const { detectPM, installCmd } = require('./pm');
...
async function removeMiddleware(name) {
    ...
    if (PROTECTED.includes(baseName)) {
        ui.abort(`'${fileName}' is a core generated file and cannot be removed via cem remove.`);
    }

    if (!fs.existsSync(filePath)) {
        ui.abort(`Middleware '${fileName}' does not exist in src/app/middlewares/`);
    }
...
async function removeEnvVar(key) {
    ...
    const ok = await confirm(
        `This will remove ${ui.bold(ui.cyan(upperKey))} from .env, .env.example, and src/app/config/index.ts.`,
    );
    ...
    if (fs.existsSync(envExamplePath)) {
        ...
        if (exampleContent !== before) {
            ...
            ui.success(`Removed ${upperKey} from .env.example`);
            removed = true;
        } else {
            ui.warn(`${upperKey} not found in .env.example`);
        }
    }
