...
function runCheck() {
    ...
    const results = {
        types:  step('Type check (tsc)…',      () => run('npx tsc --noEmit', cwd)),
        lint:   step('Lint (eslint)…',          () => run('npx eslint src', cwd)),
        format: step('Format check (prettier)…', () => run('npx prettier --check src', cwd)),
    };
    ...
    if (passed) {
        ...
        console.log();
        console.log(`  ${yellow('tip')}  ${gray('Run')} ${cyan('cem lint:fix')} ${gray('or')} ${cyan('cem format:fix')} ${gray('to auto-fix issues.')}`);
    }
