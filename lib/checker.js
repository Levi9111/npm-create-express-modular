...
const { detectPM, runScript } = require('./pm');
...
function runCheck() {
    ...
    if (passed) {
        ...
        console.log();
        const pm = detectPM();
        console.log(`  ${yellow('tip')}  ${gray('Run')} ${cyan(runScript(pm, 'lint:fix'))} ${gray('or')} ${cyan(runScript(pm, 'prettier:fix'))} ${gray('to auto-fix issues.')}`);
    }
